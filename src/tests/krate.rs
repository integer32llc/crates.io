...
fn pagination_parameters_only_accept_integers() {
    ...
    let invalid_per_page_json = anon.get_with_query::<()>("/api/v1/crates", "page=1&per_page=100%22%EF%BC%8Cexception").bad_with_status(400);
    assert_eq!(invalid_per_page_json.errors[0].detail, "invalid number");

    let invalid_page_json = anon.get_with_query::<()>("/api/v1/crates", "page=100%22%EF%BC%8Cexception&per_page=1").bad_with_status(400);
    assert_eq!(invalid_page_json.errors[0].detail, "invalid number");
}
